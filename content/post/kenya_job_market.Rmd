---
title: "Overview of Kenya's Job Market"
...
```{r, include = F}
jobs = read_csv("Kenyan-Jobs/Raw-Data/jobs_1.csv")
```
...
safaricom_grouped = read_csv("Kenyan-Jobs/Cleaned-Data/safaricom_grouped_cleaned.csv")
...
treemap(safaricom_grouped, index = c("field", "title"), vSize = "field_count", type = "index", title = "Safaricom Job Posts")
```
...
data_jobs = jobs %>%
  filter(grepl(paste(data_titles, collapse = "|"), title, ignore.case = T)) # cleaned this data using openrefine
data_jobs = read_csv("Kenyan-Jobs/Cleaned-Data/data_jobs_cleaned.csv")
...
data_jobs$title = recode(data_jobs$title, 
                         ...
                         #"Oracle Database Administrator" = "Database Adminstrator", 
                         "Senior Software Engineer - Data Visualization" = "Sr. Engineer - Visualization")
```
...
data_jobs_industry = data_jobs %>%
  group_by(industry) %>%
  summarise(industry_count = n()) %>%
  arrange(desc(industry_count)) %>%
  filter(row_number() <= 10)

data_jobs_titles = data_jobs %>%
  select(title, industry) %>%
  filter(industry %in% data_jobs_industry$industry) 

top_industry_data_jobs = data_jobs_industry %>%
  left_join(data_jobs_titles, by = "industry")
...
```{r, echo = F, fig.width=12}
treemap(top_industry_data_jobs,  index = c("industry", "title"), vSize = "industry_count", type = "index", title = "Data Posts")
```

The data and code is [here](https://github.com/florencewambui/Kenya-Job-Market).

Tools used: 

* [python](https://www.python.org/) to scrape data
* [r](https://www.r-project.org/) to clean, analyse and visualize data
* [OpenRefine](http://openrefine.org/) to clean data

Data source: [jobmag Kenya](https://www.myjobmag.co.ke/)
